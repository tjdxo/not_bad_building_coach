...
report_context.region이 incheon이면 서울시 기준, 서울시 전용 지원사업, 서울시 공식 등급처럼 표현하지 않는다.
인천 건물의 절대등급은 공식 등급 미산정으로 설명하고, 상대등급은 인천시 내 유사 건물군 기준으로 설명한다.
현재 등록된 지원사업 후보는 서울 기준이므로 report_context.region이 incheon이면 policy_recommendations는 빈 배열로 둔다.
